...
/**
 * Gestion d'un résultat Query.
 * Peut être parcouru via un foreach.
 */
class QueryResult implements \Iterator
{
   ...
   * @param  boolean $allwaysReturnArrayOfArray  [Defaut : FALSE] : Retourne toujours un tableau de tableaux, même si il n'y a qu'un résultat
   * @return type
   */
  ...
  public function valueList($column)
  {
    ...
    for ($i = 0; $i < $nb; $i++) {
      ...
      if (!is_numeric(array_search($v, $data))) {
        $data[] = $v;
      }
    }
    return $data;
  ...
  public function concat(QueryResult $qr)
  {
    ...
    if ($qr->count() == 1) {
      $this->resultat = array_merge($this->resultat, array($qr->getDatasInArray()));
    } else {
      $this->resultat = array_merge($this->resultat, $qr->getDatasInArray());
    }
  }
   ...
   *
   * @param array $data Tableau associatif contenant les données à ajouter
   */
